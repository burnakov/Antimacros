...
int Memory::GetProcessByName(LPCWSTR windowName)
{
	HWND windowHandle = FindWindowW(L"Ultima Online", windowName);
	if (windowHandle)
	{
		DWORD processID = 0;
...
short Memory::ReadShort(HANDLE processHandle, int address)
{
	if (address == -1)
		return -1;
	short buffer = 0;
